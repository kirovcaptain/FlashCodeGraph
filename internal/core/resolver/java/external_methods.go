...
// Load order: builtin → framework → user-defined (later overrides earlier).
...
func (manager *ExternalMethodManager) Lookup(className, methodName string, argTypes []string) (model.ReturnType, bool) {
	...
	// Multiple candidates: disambiguate by arg count
...
// Returns the first candidate if argTypes is nil or no exact count match.
...
// Key format: "fully.qualified.ClassName.methodName(paramType1,paramType2)"
// Rule: last "." before "(" separates class name and method name.
func parseMethodKey(key, value string) methodEntry {
	...
	if dotIndex < 0 {
		// Standalone function (no class prefix)
...
// "Function<T,R>,int" → ["Function<T,R>", "int"]
