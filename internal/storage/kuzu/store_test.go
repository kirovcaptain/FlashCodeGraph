...
func TestStoreBasicCRUD(t *testing.T) {
	...
	t.Log("✅ All KùzuDB CRUD tests passed")
}
...
func TestKuzu_MultiRelTypeRecursive(t *testing.T) {
	...
	// Now test multi-type: try Kuzu syntax [r:CALLS|DISPATCHES*1..3]
	...
	if err != nil {
		t.Logf("❌ Kuzu does NOT support multi-rel recursive: %v", err)
		t.Log("→ Need to use BFS approach for Kuzu")
	} else {
		count := 0
		for result.HasNext() {
			result.Next()
		...
		result.Close()
		t.Logf("✅ Kuzu supports multi-rel recursive: %d results", count)
	}
}

// TestKuzu_MultiRelTypeSingleHop verifies that [r:CALLS|DISPATCHES] (single hop, no *)
