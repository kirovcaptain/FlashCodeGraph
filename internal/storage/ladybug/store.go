...
func mapRelation(kind model.RelationKind, sourceKind string) (relTable, sourceLabel, targetLabel string) {
	...
	case model.RelNavigatesTo:
		if sourceKind == constants.KindFunction {
			return "NAVIGATES_TO", constants.KindFunction, constants.KindRoute
		}
		return "NAVIGATES_TO", constants.KindWidget, constants.KindWidget
	case model.RelReferences:
		sourceLabel := sourceKind
		if sourceLabel == "" {
			sourceLabel = constants.KindFunction
		}
		return "REFERENCES", sourceLabel, constants.KindWidget
